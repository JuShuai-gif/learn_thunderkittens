...
// Tile num elements constant calculated as TILE_DIM squared
template<typename T> constexpr int TILE_ELEMENTS{TILE_COL_DIM<T> * TILE_ROW_DIM<T>};

// Constant representing number of threads in a warp
constexpr int WARP_THREADS{32};

// Constant representing number of threads in a warpgroup of four warps
constexpr int WARPGROUP_THREADS{128};

// Constant representing number of warps in a warpgroup of four warps
...
#define DF_ALIGN_AS(n) __align__(n)
#else
#define DF_ALIGN_AS(n) alignas(n)
#endif

#if defined(DF_HOPPER) || defined(DF_BLACKWELL)
#define DF_DEFAULT_ALIGN DF_ALIGN_AS(128)
#else
#define DF_DEFAULT_ALIGN DF_ALIGN_AS(16)
#endif
